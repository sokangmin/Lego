...
public class FromFileBlock extends StartBlock<List<File>> {
	...
	protected List<File> doing(Void data) {
		...
		File dir = new File(PropertiesFile.getProperties().getProperty("e-hojo_contract_bcnc_dir"));
		if(!dir.isDirectory()) {
	...
	private File[] fileList(File dir){
		File[] fileArray = dir.listFiles(new FileFilter() {
			...
			public boolean accept(File file) {
				return FilenameUtils.getExtension(file.getName()).equalsIgnoreCase("txt");
			}
		});
		return fileArray;
	}
